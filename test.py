...
# ========= UTILIDADES PARA PDF/IMÁGENES (PARCHE ROBUSTO) =========
# Requiere: reportlab, pillow, kaleido
from PIL import Image as PILImage
from reportlab.lib.pagesizes import A4 as RL_A4   # alias para evitar choques de nombre
...
def build_pdf_report(title: str, hallazgos: list, image_paths: list) -> bytes:
    """Genera PDF con título, bullets e imágenes escaladas por ancho (robusto a choques A4/arrays)."""
    hallazgos = [str(h) for h in (hallazgos or []) if h is not None]
    image_paths = _valid_pngs(image_paths)

    buffer = io.BytesIO()
    # Usar RL_A4 con floats puros
    page_w, page_h = tuple(map(float, RL_A4))
    left_margin = 1.5 * float(cm)
    right_margin = 1.5 * float(cm)
    top_margin = 1.5 * float(cm)
    bottom_margin = 1.5 * float(cm)
    ...
    max_width = page_w - (left_margin + right_margin)  # todo float
