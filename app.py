...
def train_classifier(X, y):
    ...
    return model_name, model, (Xtr, Xte, ytr, yte), auc, cm, report
...
st.title("🧠 Auditoría Inteligente - Seguimiento Semana 3")
st.markdown("Creado por Juan Felipe Pinzon Trejo, David Gonzalez Idarraga y Jordan Esteban Ramirez")

with st.sidebar:
    st.header("⚙️ Configuración")
    st.write("Sube un `.csv` o usa el **dataset de demo (salud)**.")
...
if file:
    ...
    # Demo: dataset de salud que vimos
    df = pd.read_csv("dataset/Lifestyle_and_Health_Risk_Prediction_Synthetic_Dataset.csv")
else:
...
if num_cols:
    for c in num_cols:
        fig = px.histogram(df_pp, x=c, nbins=30, marginal="box", title=f"Distribución: {c}")
        st.plotly_chart(fig, use_container_width=True)
...
if "health_risk" in df_pp.columns:
    ...
    # Features = todas (one-hot limitado), quitando ID/date obvias
    ...
    if X.shape[1] >= 2 and y.sum() > 0 and y.sum() < len(y):
        model_name, model, splits, auc, cm, report = train_classifier(X, y)
...
if len(num_for_cluster) >= 2:
    ...
    st.subheader("PCA 2D (KMeans)")
    st.plotly_chart(px.scatter(df_clu, x="PC1", y="PC2", color="KMeans"), use_container_width=True)

    st.subheader("PCA 2D (DBSCAN)")
    st.plotly_chart(px.scatter(df_clu, x="PC1", y="PC2", color="DBSCAN"), use_container_width=True)
else:
...
if "health_risk" in df_pp.columns:
    ...
    if sensitive_col != "(no disponible)":
        ...
        if not fair.empty:
            st.dataframe(fair.sort_values("positive_rate", ascending=False))
...
if HAS_PROPHET:
    if metric_for_prophet in df_pp.columns and df_pp[metric_for_prophet].dtype.kind in "if":
        ...
        if len(daily) >= 10:
            ...
            st.plotly_chart(
                px.line(daily, x="ds", y="y", title=f"{metric_for_prophet} promedio diario (histórico sintético)"),
...
# Reporte
st.header("8) Reporte automático")
hallazgos = []
...
try:
    if "disparate_impact" in fair.columns:
        peor = fair["disparate_impact"].idxmax()
...
# Prophet
try:
    if HAS_PROPHET and len(daily) > 0:
...
st.download_button(
    ...
    file_name="reporte_auditoria.md",
    mime="text/markdown"
)

st.caption("© Auditoría Inteligente – Demo")
